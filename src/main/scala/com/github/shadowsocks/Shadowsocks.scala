...
object Shadowsocks {
  ...
  val PROXY_PREFS = Array(Key.profileName, Key.proxy, Key.remotePort, Key.localPort, Key.sitekey,
    Key.encMethod)
  val FEATRUE_PREFS = Array(Key.isGFWList, Key.isZJUList, Key.isGlobalProxy, Key.proxyedApps, Key.isTrafficStat,
  ...
  def updatePreference(pref: Preference, name: String, profile: Profile) {
    name match {
      ...
      case Key.isZJUList => updateCheckBoxPreference(pref, profile.zjuroute)
